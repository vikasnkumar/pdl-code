...
sub importer {
    ...
    return sub {
        ...
        shift(@_);
        DEBUG "Inline::Module::importer proxy to Inline::%s", @_;
...
sub postamble {
    my ($makemaker, %args) = @_;
    DEBUG "Inline::Module::postamble(@_)";
...
sub handle_makestub {
    my ($class, @args) = @_;
    DEBUG "Inline::Module::handle_makestub(@_)";
...
sub handle_distdir {
    ...
    DEBUG "Inline::Module::handle_distdir(@_)";
    my $stub_modules = [];
    my $included_modules = [];
...
sub handle_makeblibdyna {
    my ($class, $module) = @_;
    DEBUG "$class->handle_makeblibdyna($module)";
...
sub included_modules {
    my ($class, $meta) = @_;
    DEBUG "Inline::Module::included_modules(@_)";
...
sub add_to_distdir {
    ...
    DEBUG "$class->add_to_distdir($distdir) [@$stub_modules] [@$included_modules]";
...
sub make_stub_modules {
    my ($class, @modules) = @_;
    DEBUG "$class->make_stub_modules(@modules)";
...
sub proxy_module {
    my ($class, $module) = @_;
    DEBUG "$class->proxy_module($module)";
...
sub dyna_module {
    my ($class, $module) = @_;
    DEBUG "$class->dyna_module($module)";
    return <<"...";
...
sub read_file {
    my ($class, $filepath) = @_;
    DEBUG "$class->read_file($filepath)";
    open IN, '<', $filepath
...
sub add_to_manifest {
    ...
    DEBUG "$class->add_to_manifest($distdir) (@files)";
